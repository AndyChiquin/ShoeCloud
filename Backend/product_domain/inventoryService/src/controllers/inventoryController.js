...
const createInventory = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Inventario creado', data: { id, product_id, quantity } });
  } catch (error) {
    res.status(500).json({ error: 'Error al crear inventario', details: error });
  }
};
...
const deleteInventory = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Error al eliminar inventario', details: error });
  }
};
